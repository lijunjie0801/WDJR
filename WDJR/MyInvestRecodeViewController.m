//
//  MyInvestRecodeViewController.m
//  WDJR
//
...
@interface MyInvestRecodeViewController ()<UITextFieldDelegate>
{
    int pageNum;
}
...
@property(nonatomic, strong) NSMutableArray *myInvesterArray;

@end

@implementation MyInvestRecodeViewController
...
- (void)viewDidLoad {
    ...
    _myInvesterArray = [NSMutableArray array];
...
-(void)requestRefresh
{
    ...
    pageNum =1;
    [_myInvesterArray removeAllObjects];
    [self.tableView reloadData];
    
    [_svc  showLoadingWithMessage:@"加载中..." inView:[UIApplication sharedApplication].keyWindow ];
    [MyInvesterModel requestMyInvesterModelStartDate:self.startDateTtx.text andEndData:self.endDateTxt.text andPage:pageNum SuccessHandle:^(id object) {
        
        
        NSArray *array = (NSArray *)object;
        
        [_myInvesterArray addObjectsFromArray:array];
        [self finishRequest];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    MyInvesterModel *model = _myInvesterArray[indexPath.row];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    MyInvesterModel *model = _myInvesterArray[indexPath.row];
    [_svc pushViewController:_svc.myInvestDetailesViewController withObjects:@{@"MyInvesterModel":model,@"typeNum":@0}];
}
