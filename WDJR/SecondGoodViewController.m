...
@interface SecondGoodViewController ()<UITextFieldDelegate>
{
    int pageNum;
}
...
@property(nonatomic, strong) NSMutableArray *secondInvesterArray;

@end

@implementation SecondGoodViewController
...
- (void)viewDidLoad {
    ...
    _secondInvesterArray = [NSMutableArray array];
...
-(void)requestRefresh
{
    ...
    [_secondInvesterArray removeAllObjects];
    pageNum =1;
    [self.tableView reloadData];
    [_svc  showLoadingWithMessage:@"加载中..." inView:[UIApplication sharedApplication].keyWindow ];
    [SecondInvesterModel requestSecondInvesterModelStartDate:self.startDateTtx.text andEndData:self.endDateTxt.text andPage:pageNum SuccessHandle:^(id object) {
        
        NSArray *array = (NSArray *)object;
        
        [_secondInvesterArray addObjectsFromArray:array];
        [self finishRequest];
...
{
    return self.secondInvesterArray.count;
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    SecondInvesterModel *model =_secondInvesterArray[indexPath.row];
    ...
    if ([model.status isEqualToString:@"Y"]) {
         ...
         cell.stateslbl.text = @"状态：已完成";
    }else if ([model.status isEqualToString:@"S"]){
    
        cell.stateslbl.text = @"状态：回款处理中";
    }
    //cell.stateslbl.text = [NSString stringWithFormat:@"状态：%@",model.status];
...
{
    SecondInvesterModel *model =_secondInvesterArray[indexPath.row];
    [_svc pushViewController:_svc.myInvestDetailesViewController withObjects:@{@"SecondInvesterModel":model,@"typeNum":@1}];
}
