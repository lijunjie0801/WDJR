...
@interface JiFenRecordViewController ()<UITextFieldDelegate>
{
    int pageNum;
}
...
@property(nonatomic, strong) NSMutableArray *jifenModelArray;
...
- (void)viewDidLoad {
    ...
    _jifenModelArray = [[NSMutableArray alloc] init];
...
{
    pageNum = 1;
    [_jifenModelArray removeAllObjects];
    [_tableView reloadData];
    [_svc  showLoadingWithMessage:@"加载中..." inView:[UIApplication sharedApplication].keyWindow ];
    
    [JiFenRecordModel requestJiFenRecordModelStartDate:self.startDateTtx.text andEndData:self.endDateTxt.text andPage:pageNum SuccessHandle:^(id object) {
        
        NSArray *array = (NSArray *)object;
        
        [_jifenModelArray addObjectsFromArray:array];
        [self finishRequest];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    JiFenRecordModel *model = _jifenModelArray[indexPath.row];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    JiFenRecordModel *model = _jifenModelArray[indexPath.row];
    [_svc pushViewController:_svc.moneyRecordDetailViewController withObjects:@{@"JiFenRecordModel":model,@"typeNum":@3}];
}
