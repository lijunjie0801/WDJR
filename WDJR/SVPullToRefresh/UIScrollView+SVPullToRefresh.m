...
- (void)layoutSubviews {
    ...
    if(hasCustomView) {
        ...
        //IOS7OrPlus
        CGSize size         = CGSizeMake(labelMaxWidth,self.titleLabel.font.lineHeight);
        NSDictionary * tdic = [NSDictionary dictionaryWithObjectsAndKeys:self.titleLabel.font,NSFontAttributeName,nil];
        CGSize titleSize =[self.titleLabel.text boundingRectWithSize:size options:NSStringDrawingUsesLineFragmentOrigin  attributes:tdic context:nil].size;

        size                = CGSizeMake(labelMaxWidth,self.subtitleLabel.font.lineHeight);
        NSDictionary * sdic = [NSDictionary dictionaryWithObjectsAndKeys:self.subtitleLabel.font,NSFontAttributeName,nil];
        CGSize subtitleSize =[self.subtitleLabel.text boundingRectWithSize:size options:NSStringDrawingUsesLineFragmentOrigin  attributes:sdic context:nil].size;
...
- (SVPullToRefreshArrow *)arrow {
    if(!_arrow) {
        ...
        _arrow.arrowColor = [AppAppearance sharedAppearance].mainColor;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
//        SpinView* spinner = [[SpinView alloc] initWithFrame:self.bounds];
