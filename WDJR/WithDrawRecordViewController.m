...
@property(nonatomic, strong) NSMutableArray *withDrawModelArray;
@end
...
- (void)viewDidLoad {
    ...
    _withDrawModelArray = [[NSMutableArray alloc] init];
...
{
    pageNum = 1;
    [_withDrawModelArray removeAllObjects];
    [_tableView reloadData];
    [_svc  showLoadingWithMessage:@"加载中..." inView:[UIApplication sharedApplication].keyWindow ];
    
    [WithDrawRecordModel requestWithDrawRecordModelStartDate:self.startDateTtx.text andEndData:self.endDateTxt.text andPage:pageNum SuccessHandle:^(id object) {
        
        NSArray *array = (NSArray *)object;
        
        [_withDrawModelArray addObjectsFromArray:array];
        [self finishRequest];
...
{
    return _withDrawModelArray.count;
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    WithDrawRecordModel *model = _withDrawModelArray[indexPath.row];
    
    cell.titlelbl.text = [NSString stringWithFormat:@"提现银行：%@",model.bank_name];
    cell.moneylbl.text = [NSString stringWithFormat:@"提现金额：%@元",model.account];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    WithDrawRecordModel *model = _withDrawModelArray[indexPath.row];
    [_svc pushViewController:_svc.moneyRecordDetailViewController withObjects:@{@"WithDrawRecordModel":model,@"typeNum":@2}];
}
