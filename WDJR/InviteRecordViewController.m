...
@interface InviteRecordViewController ()<UITextFieldDelegate>
{
    int pageNum;
}
...
@property(nonatomic, strong) NSMutableArray *inviteRecordArray;
@end
...
- (void)viewDidLoad {
    ...
    _inviteRecordArray = [[NSMutableArray alloc] init];
...
{
    pageNum = 1;
    [_inviteRecordArray removeAllObjects];
    [_tableView reloadData];
    [_svc  showLoadingWithMessage:@"加载中..." inView:[UIApplication sharedApplication].keyWindow ];
    
    [InviteRecordModel requestInviteRecordModelStartDate:self.startDateTtx.text andEndData:self.endDateTxt.text andPage:pageNum SuccessHandle:^(id object) {
        
        NSArray *array = (NSArray *)object;
        
        [_inviteRecordArray addObjectsFromArray:array];
        [self finishRequest];
...
{
    return _inviteRecordArray.count;
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    InviteRecordModel *model = _inviteRecordArray[indexPath.row];
    
    cell.titlelbl.text = [NSString stringWithFormat:@"邀请时间：%@",model.created_at];
    cell.moneylbl.text = [NSString stringWithFormat:@"好友：%@",model.username];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    InviteRecordModel *model = _inviteRecordArray[indexPath.row];
    [_svc pushViewController:_svc.moneyRecordDetailViewController withObjects:@{@"InviteRecordModel":model,@"typeNum":@5}];
}
