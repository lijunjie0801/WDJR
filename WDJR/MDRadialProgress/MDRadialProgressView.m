...
- (void)internalInitWithTheme:(MDRadialProgressTheme *)theme
{
    ...
    self.JinDulabel = [[MDRadialProgressLabel alloc] initWithFrame:self.bounds andTheme:self.theme];
...
-(void)setWanChen:(NSString *)wanChen
{
    _wanChen = wanChen;
    CGRect labelRect = self.label.frame;
    
    if ([self.wanChen isEqualToString:@"完成进度"]) {
        
        labelRect.origin.y-=5;
        
        self.label.frame = labelRect;
        
        self.JinDulabel.text = @"完成进度";
        self.JinDulabel.textAlignment = NSTextAlignmentCenter;
        self.JinDulabel.textColor = [AppAppearance sharedAppearance].title2TextColor;
        self.JinDulabel.font = shierFont;
        
        CGPoint point= self.label.center;
        
        self.JinDulabel.frame = CGRectMake(point.x-30, point.y+10, 60, 21);
    }
}

#pragma mark - Setters

- (void)setProgressCounter:(CGFloat)progressCounter
{
...
- (void)setProgressTotal:(CGFloat)progressTotal
{
...
- (void)notifyProgressChange
{
    ...
    if (self.progressCounter != 0) {
        ...
        self.accessibilityValue = [NSString stringWithFormat:@"0"];
