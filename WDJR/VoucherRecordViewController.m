...
@interface VoucherRecordViewController ()<UITextFieldDelegate>
{
    int pageNum;
}
...
@property(nonatomic, strong) NSMutableArray *voucherRecordArray;
@end

@implementation VoucherRecordViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    _voucherRecordArray = [[NSMutableArray alloc] init];
...
{
    pageNum = 1;
    [_voucherRecordArray removeAllObjects];
    [_tableView reloadData];
    [_svc  showLoadingWithMessage:@"加载中..." inView:[UIApplication sharedApplication].keyWindow ];
    
    [VoucherRecordModel requestVoucherRecordModelStartDate:self.startDateTtx.text andEndData:self.endDateTxt.text andPage:pageNum SuccessHandle:^(id object) {
        
        NSArray *array = (NSArray *)object;
        
        [_voucherRecordArray addObjectsFromArray:array];
        [self finishRequest];
...
{
    return _voucherRecordArray.count;
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    VoucherRecordModel *model = _voucherRecordArray[indexPath.row];
    
    cell.titlelbl.text = [NSString stringWithFormat:@"时间：%@",model.created_at];
    cell.moneylbl.text = [NSString stringWithFormat:@"数额：%@",model.amount];
    
    cell.stateslbl.text = [NSString stringWithFormat:@"类型：%@",model.name];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    VoucherRecordModel *model = _voucherRecordArray[indexPath.row];
    [_svc pushViewController:_svc.moneyRecordDetailViewController withObjects:@{@"VoucherRecordModel":model,@"typeNum":@4}];
}
