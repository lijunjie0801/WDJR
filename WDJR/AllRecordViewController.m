...
@interface AllRecordViewController ()<UITextFieldDelegate>
...
-(void)requestRefresh
{
    ...
    [AllRecordModel requestAllRecordModelStartDate:self.startDateTtx.text andEndData:self.endDateTxt.text andPage:pageNum SuccessHandle:^(id object) {
        
        NSArray *array = (NSArray *)object;
        
        [_allModelArray addObjectsFromArray:array];
        [self finishRequest];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    AllRecordModel *model = _allModelArray[indexPath.row];
    
    cell.titlelbl.text = [NSString stringWithFormat:@"时间：%@",model.created_at];
    cell.moneylbl.text = [NSString stringWithFormat:@"金额：%@元",model.amount];
    
    cell.stateslbl.text = [NSString stringWithFormat:@"类型：%@",model.type];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    AllRecordModel *model = _allModelArray[indexPath.row];
    [_svc pushViewController:_svc.moneyRecordDetailViewController withObjects:@{@"AllRecordModel":model,@"typeNum":@0}];
